 ...
 *
 */
	public class NotEqual implements IComparisonOperator {

	/**
	 * Returns true if value1 for each element is not equal than
	...
	public boolean satisfied(String value1, String value2) {
		...
		for (int i = 0; i < splitted1.length; i++) {
			for (int j = 0; j < splitted2.length; j++) {
				if ((Normalizer
						.normalize(splitted1[i], Normalizer.Form.NFD)
						.equals(Normalizer.normalize(splitted2[j],
								Normalizer.Form.NFD)))) {
					return false;
				}
			}
