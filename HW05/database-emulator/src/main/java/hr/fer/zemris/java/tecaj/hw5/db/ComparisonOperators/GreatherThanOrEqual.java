 ...
 *
 */
public class GreatherThanOrEqual implements IComparisonOperator {

	/**
	 * Returns true if value1 for each element is greather or equal than
	...
	public boolean satisfied(String value1, String value2) {
		...
		for (int i = 0; i < splitted1.length; i++) {
			for (int j = 0; j < splitted2.length; j++) {
				if (!(Normalizer.normalize(splitted1[i], Normalizer.Form.NFD)
						.compareTo(
								Normalizer.normalize(splitted2[j],
										Normalizer.Form.NFD)) >= 0)) {
					return false;
				}
			}
		}
