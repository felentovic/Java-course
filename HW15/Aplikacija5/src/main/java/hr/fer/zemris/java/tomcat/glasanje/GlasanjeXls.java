...
public class GlasanjeXls extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		List<Stavka> rezultati = (List<Stavka>) getServletContext()
	 ...
	 */
	private HSSFWorkbook generateExcelFile(List<Stavka> rezultati)
			throws IOException {
		...
		for (Stavka rez : rezultati) {
			HSSFRow row = sheet.createRow(brojacRed);
			row.createCell(0).setCellValue(rez.getNaziv());
